...
<div class="flex wrapper">
    ...
    <div class="page-content">
        ...
        $answerC = new answerC();
        if (isset($_POST["validate"]) && isset($_GET['idQuestion'])) {
            if (!empty($_POST["contenu"]) && !empty($_GET['idQuestion'])) {
                $idQuestion = $_GET['idQuestion'];
                $answer1 = new answer(NULL, $_POST["contenu"], 1, date('Y-m-d H:i:s'), $idQuestion);
                $answerC->createAnswer($answer1);
                // Correct the concatenation within the script tag
                echo '<script>window.location.href = "viewAnswers.php?idQuestion=' . $idQuestion . '";</script>';
            }
        }
        ?>
